...
/* lock while push message to daemon  */
...
static void print_rules(void)
{
	...
	list_for_each_entry(current_rule, &(rules.list), list)
	{
		if (current_rule->allowed) {
			printk(KERN_INFO
				"douane:%d:%s: process_path %s is allowed.\n",
				__LINE__, __FUNCTION__,
				current_rule->process_path);
		} else {
			printk(KERN_INFO
				"douane:%d:%s: process_path %s is disallowed.\n",
				__LINE__, __FUNCTION__,
				current_rule->process_path);
		}
	}
...
static void append_rule(const char *process_path, const bool is_allowed)
{
	...
	new_rule = (struct rule *) kmalloc(sizeof(struct rule), GFP_KERNEL);
	if (new_rule == NULL) {
	...
	if (new_rule->allowed) {
		printk(KERN_INFO
			"douane:%d:%s: New rule of type ALLOW added for %s\n",
			__LINE__, __FUNCTION__, new_rule->process_path);
	} else {
		printk(KERN_INFO
			"douane:%d:%s: New rule of type DISALLOW added for %s\n",
			__LINE__, __FUNCTION__, new_rule->process_path);
	}
	print_rules();
#endif
...
static struct rule *search_rule_for_process_path(const unsigned char *process_path)
{
	...
	list_for_each_entry(current_rule, &(rules.list), list)
	{
		...
		if (strcmp(current_rule->process_path, process_path) == 0) {
			...
			if (current_rule->allowed) {
				printk(KERN_INFO
					"douane:%d:%s: Rule ALLOW found for %s.\n",
					__LINE__, __FUNCTION__,
					current_rule->process_path);
			} else {
				printk(KERN_INFO
					"douane:%d:%s: Rule DISALLOW found for %s.\n",
					__LINE__, __FUNCTION__,
					current_rule->process_path);
			}
#endif
...
static void remember_process_socket_inode(const pid_t pid,
	const uint32_t sequence,
	const unsigned long i_ino,
	const char *path)
{
	...
	/* In order to clean this linked list, before saving the new relation
	 * process <-> socket file we first look if there are records for the
	 * given PID with a different path.
	 * If this is the case it means that it is a new process that own the
	 * given PID so previous records can be cleaned
	 */
	list_for_each_entry_safe(process_socket_inode_a, process_socket_inode_b,
		&(process_socket_inodes.list), list)
	{
		if (process_socket_inode_a->pid == pid
			&& strcmp(process_socket_inode_a->process_path,
			path) != 0) {
#ifdef DEBUG
			printk(KERN_INFO "douane:%d:%s: The process path %s "
				"relates to PID %d but is now %s.\n",
				__LINE__, __FUNCTION__,
				process_socket_inode_a->process_path,
				process_socket_inode_a->pid, path);
	...
	if (process_cleaned_records > 0) {
		printk(KERN_INFO "douane:%d:%s: %d process <-> socket file "
	 ...
	 */
	new_process_socket_inode = (struct process_socket_inode *)
		kmalloc(sizeof(struct process_socket_inode), GFP_KERNEL);
	if (new_process_socket_inode == NULL) {
		printk(KERN_ERR "douane:%d:%s: Failed to allocate new "
	...
	list_add_tail(&(new_process_socket_inode->list),
		&(process_socket_inodes.list));
}

static void forget_process_socket_inode(const pid_t pid,
	const unsigned long i_ino)
{
	struct process_socket_inode *process_socket_inode_a,
		*process_socket_inode_b;
	...
	{
		if (process_socket_inode_a->pid == pid
			&& process_socket_inode_a->i_ino == i_ino) {
			list_del(&process_socket_inode_a->list);
			kfree(process_socket_inode_a);
		}
	}
...
static void update_task_info_sequence(const unsigned long i_ino,
	const uint32_t sequence)
{
	...
	if (i_ino != 0) {
		...
		list_for_each_entry(current_process_socket_inode,
			&(process_socket_inodes.list), list)
		{
			if (current_process_socket_inode->i_ino == i_ino) {
				current_process_socket_inode->sequence =
					sequence;
			}
		}
	}
...
static struct process_socket_inode *task_info_from_open_file_inode(const
	unsigned long
	i_ino)
{
	struct process_socket_inode *current_process_socket_inode;
...
static struct process_socket_inode *task_info_from_sequence(const uint32_t
sequence)
{
	struct process_socket_inode *current_process_socket_inode;
		...
		&(process_socket_inodes.list), list)
	{
		if (current_process_socket_inode->sequence > 0) {
			if (current_process_socket_inode->sequence == sequence
				|| ((current_process_socket_inode->sequence + 1) ==
				sequence)) {
				return current_process_socket_inode;
			}
		}
...
static void clear_process_socket_inodes(void)
{
	...
	while (!list_empty(&process_socket_inodes.list)) {
		iter =
			list_entry(process_socket_inodes.list.next,
			struct process_socket_inode, list);
		list_del(&iter->list);
		kfree(iter);
...
#ifdef DEBUG
	printk(KERN_INFO
		"douane:%d:%s: %d relations process <-> socket file successfully cleaned.\n",
...
/* Netfilter socket to send network activities and recieve orders from the user space daemon. */
static struct sock *activities_socket = NULL;
...
static int push(const struct network_activity *activity)
{
	...
	if (activities_socket == NULL) {
		printk(KERN_ERR
			"douane:%d:%s: BLOCKED PUSH: Socket not connected!!.\n",
			__LINE__, __FUNCTION__);
		return -1;
	}

	/* If no process_path don't send the network_activity message to the daemon */
	if (activity->process_path == NULL
		|| strcmp(activity->process_path, "") == 0) {
		printk(KERN_ERR
			"douane:%d:%s: BLOCKED PUSH: process_path is blank.\n",
			__LINE__, __FUNCTION__);
		return 0;
	}
	...
	if (ret < 0) {
		if (ret == -11) {
			printk(KERN_WARNING
				"douane:%d:%s: Message ignored as Netfiler socket is busy.\n",
				__LINE__, __FUNCTION__);
		} else {
			printk(KERN_ERR
				"douane:%d:%s: Failed to send message (errno: %d).\n",
...
#ifdef DEBUG
		printk(KERN_INFO
			"douane:%d:%s: Shutting down activities Netlink socket...\n",
			__LINE__, __FUNCTION__);
#endif
...
static void activities_socket_receiver(struct sk_buff *skb)
{
	...
	if (activity == NULL) {
		printk(KERN_ERR
			"douane:%d:%s: Can't allocate memory for a new activity\n",
			__LINE__, __FUNCTION__);
		return;
	}
	...
	if (activity->kind <= 0) {
		printk(KERN_ERR
			"douane:%d:%s: Invalid received message. (No kind)\n",
			__LINE__, __FUNCTION__);
...
#ifdef DEBUG
		printk(KERN_INFO
			"douane:%d:%s: Process %d successfully registered.\n",
	...
	case KIND_SENDING_RULE:
	{
		...
		new_rule =
			search_rule_for_process_path
			(activity->process_path);
		/* No rule found for the process path */
		if (new_rule == NULL) {
			/* Create the new rule */
			append_rule(activity->process_path,
				(activity->allowed == 1));
		}
	}
		break;
	...
	{
		printk(KERN_ERR
			"douane:%d:%s: Invalid received message. (Unknown kind)\n",
			__LINE__, __FUNCTION__);
...
static int initialize_activities_socket(void)
{
	...
	if (activities_socket == NULL) {
		printk(KERN_ERR
			"douane:%d:%s: Can't create Netlink socket for network activities!\n",
			__LINE__, __FUNCTION__);
		return -ENOMEM;
	}
 ...
 */
static bool task_has_open_file(const struct task_struct *task,
	const struct file *socket_file)
{
...
#ifdef DEBUG
					printk(KERN_INFO
						"douane:%d:%s: fd_null_max: %d | MAX_FD_NULL: %d\n",
						__LINE__, __FUNCTION__,
						fd_null_max, MAX_FD_NULL);
#endif
					return true;
				}
			}
	...
	do {
		...
		if (likely(locked_task->mm)) {
			if (locked_task->mm->exe_file) {
				 ...
				 */
				strcpy(buff,
					d_path(&locked_task->mm->
					exe_file->f_path, tmpbuf,
					PATH_LENGTH));
...
static void print_tcp_packet(const struct tcphdr *tcp_header,
	const struct iphdr *ip_header,
	char *ip_source, char *ip_destination,
	char *process_path)
{
	if (tcp_header) {
		printk(KERN_INFO
			"douane:%d:%s: TCP | [seq:%u|ack_seq:%u]\tFLAGS=%c%c%c%c%c%c | TGID %d:\t%s:%hu --> %s:%hu (%s|%s) {W:%hu|%d} %s.\n",
			...
			tcp_header->rst ? 'R' : '-',
			tcp_header->syn ? 'S' : '-',
			tcp_header->fin ? 'F' : '-', current->tgid,
			ip_source, ntohs(tcp_header->source),
			ip_destination, ntohs(tcp_header->dest),
			process_path, current->comm,
			ntohs(tcp_header->window),
			ntohs(ip_header->tot_len) -
			(tcp_header->doff * 4) - (ip_header->ihl * 4),
			"NF_ACCEPT");
	}
}
#endif
	...
	int (*okfn) (struct sk_buff *))
{
	...
	if (skb == NULL) {
		printk(KERN_WARNING
			"douane:%d:%s: Allowing traffic as socket buffer is NULL.\n",
			__LINE__, __FUNCTION__);
		kfree(activity);
		return NF_ACCEPT;
	}
	...
	if (ip_header == NULL) {
		printk(KERN_ERR
			"douane:%d:%s: !!OOPS!! ip_header is NULL. !!OOPS!!\n",
			__LINE__, __FUNCTION__);
		kfree(activity);
		return NF_ACCEPT;
	}
	...
	case IPPROTO_UDP:
	{
		...
		if (udp_header == NULL) {
			printk(KERN_ERR
				"douane:%d:%s: !!OOPS!! udp_header is NULL. !!OOPS!!\n",
				__LINE__, __FUNCTION__);
			kfree(activity);
			return NF_ACCEPT;
		}
	...
	case IPPROTO_TCP:
	{
		...
		if (tcp_header == NULL) {
			printk(KERN_ERR
				"douane:%d:%s: !!OOPS!! tcp_header is NULL. !!OOPS!!\n",
				__LINE__, __FUNCTION__);
			kfree(activity);
			return NF_ACCEPT;
		}
	...
	if (skb->sk) {
		if (skb->sk->sk_socket) {
			if (skb->sk->sk_socket->file) {
				...
				task_info =
					task_info_from_open_file_inode
					(socket_file_ino);
				if (task_info) {
					if (tcp_header)
					...
					if (task) {
							...
							if (tcp_header == NULL ||
								...
								if (task_with_open_file) {
									...
									if (tcp_header) {
										...
										remember_process_socket_inode
											(task_with_open_file->pid,
											ntohl
											(tcp_header->seq),
											socket_file_ino,
											process_owner_path);
						...
						/* Task no more exists */
						printk(KERN_INFO
							"douane:%d:%s: !!OOO!! Task no more exists. !!OOO!!\n",
							__LINE__,
							__FUNCTION__);
#endif
					}
					...
					if (task) {
						...
						printk(KERN_WARNING
							"douane:%d:%s: !!OOPS!! Don't know and can't find a task for socket file inode number %ld !!OOPS!!\n",
							__LINE__,
							__FUNCTION__,
							socket_file_ino);
#endif
...
#ifdef DEBUG
				printk(KERN_WARNING
					"douane:%d:%s: !!WARN WARN WARN!! skb->sk->sk_socket->file is NULL. !!WARN WARN WARN!!\n",
					__LINE__, __FUNCTION__);
#endif
...
#ifdef DEBUG
			printk(KERN_WARNING
				"douane:%d:%s: !!WARN WARN WARN!! skb->sk->sk_socket is NULL. !!WARN WARN WARN!!\n",
				__LINE__, __FUNCTION__);
#endif
			lookup_from_socket_sequence = true;
		}
...
#ifdef DEBUG
		printk(KERN_WARNING
			"douane:%d:%s: !!WARN WARN WARN!! skb->sk is NULL. !!WARN WARN WARN!!\n",
			__LINE__, __FUNCTION__);
#endif
		lookup_from_socket_sequence = true;
	}
	...
	if (lookup_from_socket_sequence) {
		if (tcp_header) {
			...
			if (task_info) {
				...
				printk(KERN_ERR
					"douane:%d:%s: !!OOPS!! skb->sk->sk_socket or skb->sk is NULL and nothing found from sequence %u. !!OOPS!!\n",
					__LINE__, __FUNCTION__,
			...
			printk(KERN_ERR
				"douane:%d:%s: !!OOPS!! lookup_from_socket_sequence but wasn't a TCP packet! !!OOPS!!\n",
				__LINE__, __FUNCTION__);
#endif
		}
	}
#ifdef DEBUG
	print_tcp_packet(tcp_header, ip_header, ip_source,
	ip_destination, process_owner_path);
#endif

	/* When was filterable but didn't got process path */
	if (filterable
		&& (process_owner_path == NULL
		|| strcmp(process_owner_path, "") == 0)) {
		kfree(activity);
#ifdef DEBUG
		printk(KERN_INFO
			"douane:%d:%s: !!OOPS!! process_owner_path is blank but was filterable! !!OOPS!!\n",
			__LINE__, __FUNCTION__);
#endif
		return NF_ACCEPT;
	}
...
#ifdef DEBUG
		printk(KERN_INFO
			"douane:%d:%s: NF_ACCEPT (No daemon).\n",
			__LINE__, __FUNCTION__);
#endif
		return NF_ACCEPT;
	}
	...
	if (filterable) {
...
#ifdef DEBUG
			printk(KERN_INFO
				"douane:%d:%s: NF_QUEUE for %s as no rules yet.\n",
				__LINE__, __FUNCTION__,
				process_owner_path);
#endif
			return NF_QUEUE;
...
#ifdef DEBUG
				printk(KERN_INFO
					"douane:%d:%s: NF_ACCEPT for %s as allowed.\n",
					__LINE__, __FUNCTION__,
					process_owner_path);
#endif
				return NF_ACCEPT;
...
#ifdef DEBUG
				printk(KERN_INFO
					"douane:%d:%s: NF_DROP for %s as disallowed.\n",
					__LINE__, __FUNCTION__,
					process_owner_path);
#endif
				return NF_DROP;
		...
		printk(KERN_INFO
			"douane:%d:%s: NF_ACCEPT for %s as not filterable.\n",
...
static int __init initialize_module(void)
{
	...
	if (initialize_activities_socket() < 0) {
		printk(KERN_ERR
			"douane:%d:%s: Unable to create Netlink socket!\n",
			__LINE__, __FUNCTION__);
